...
object Main:
  ...
  def render =
    ...
    val text = div("div").child(
      ...
      p("NB: It's probably easiest to reason about the ambiguity in the <code>Beam</code> and <code>Curve</code> examples. In these cases, when the end vertices (right and top, respectively) are moved, ambiguity can clearly be seen in the interim vertices, which are controlled by a number of non-orthogonal parameters. When the interior vertices are moved, the ends are ambiguous: do they stay the same, or do we preserve the length of the parameters from the interim to the end?")
    ).withClass("explain")
